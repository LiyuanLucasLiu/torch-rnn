...
local layer, parent = torch.class('nn.LLSTM', 'nn.Module')
...
function layer:updateOutput(input)
  ...
  if not h0 then
    ...
    if h0:nElement() == 0 or not self.remember_states then
      h0:resize(N, H):zero()
    elseif self.remember_states then
      local prev_N, prev_T = self.output_rem:size(1), self.output_rem:size(2)
      assert(prev_N == N, 'batch sizes must be the same to remember states')
      h0:copy(self.output_rem[{{}, prev_T}])
    end
  end
  ...
  local h, c = self.output_rem, self.cell
  h:resize(N, T, H):zero()
  c:resize(N, T, H):zero()
