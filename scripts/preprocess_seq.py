...
if __name__ == '__main__':
  ...
  train = np.zeros(train_size, max_length, dtype=dtype)
  val = np.zeros(val_size, max_length, dtype=dtype)
  test = np.zeros(test_size, max_length, dtype=dtype)
  splits = [train, val, test]

  # Go through the file again and write data to numpy arrays
  split_idx, cur_idx, sen_idx = 0, 0, 0
  with codecs.open(args.input_txt, 'r', args.encoding) as f:
    for line in f:
      cur_idx = 0
      for char in line:
        splits[split_idx][sen_idx][cur_idx] = token_to_idx[char]
        cur_idx += 1
      sen_idx += 1
