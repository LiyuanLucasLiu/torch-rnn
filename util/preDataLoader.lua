...
local preDataLoader = torch.class('preDataLoader')


function preDataLoader:__init(kwargs, num4padding)
  ...
  for idx=1,num4padding do
    ...
    self.x_splits['train'][idx] = vx[{{1, tmpsize}, {1, -2}}]:view(tmpsize/self.batch_size, self.batch_size, -1):clone() 
    self.y_splits['train'][idx] = vx[{{1, tmpsize}, {2, -1}}]:view(tmpsize/self.batch_size, self.batch_size, -1):clone() 
    ...
    tmpsize = vx:size(1)
    tmpsize = tmpsize - tmpsize%self.batch_size
    self.x_splits['val'][idx] = vx[{{1, tmpsize}, {1, -2}}]:view(tmpsize/self.batch_size, self.batch_size, -1):clone()
    self.y_splits['val'][idx] = vx[{{1, tmpsize}, {2, -1}}]:view(tmpsize/self.batch_size, self.batch_size, -1):clone()
    ...
    tmpsize = vx:size(1)
    tmpsize = tmpsize - tmpsize%self.batch_size
    self.x_splits['test'][idx] = vx[{{1, tmpsize}, {1, -2}}]:view(tmpsize/self.batch_size, self.batch_size, -1):clone()
    self.y_splits['test'][idx] = vx[{{1, tmpsize}, {2, -1}}]:view(tmpsize/self.batch_size, self.batch_size, -1):clone()
...
function preDataLoader:nextBatch(split)
