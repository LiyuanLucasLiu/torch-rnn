...
local LM, parent = torch.class('nn.LanguageModel_enc', 'nn.Module')
...
function LM:__init(kwargs)
  ...
  self.middle_dim = utils.get_kwarg(kwargs, 'middle_dim')
  ...
  for i = 1, self.num_layers do
    ...
    if i == self.num_layers then after_dim = middle_dim end
    local rnn
    if self.model_type == 'rnn' then
      rnn = nn.VanillaRNN(prev_dim, after_dim)
    elseif self.model_type == 'lstm' then
      rnn = nn.PLSTM(prev_dim, after_dim)
    end
...
function LM:backwardConnect(decoder)
  for i = 1, self.num_layers do
    self.rnns[i].grad_h0 = decoder.rnns[i].grad_h0
    self.rnns[i].grad_c0 = decoder.rnns[i].grad_c0
  end
end
-- --[[
